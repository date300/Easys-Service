...
class ProfileScreen extends ConsumerWidget {
  ...
  Widget _buildThemeSelectorCard(BuildContext context, WidgetRef ref, bool isDark) {
    ...
    final primaryColor = isDark ? skyBlue : skyBlue;
  ...
  Widget _buildProfileItem(BuildContext context, IconData icon, String title, {required bool isDark, Color? iconColor, Color? textColor, required VoidCallback onTap}) {
    return Card(
      ...
      color: isDark ? const Color(0xFF1E1E1E) : Colors.white,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12.r), side: isDark ? const BorderSide(color: Color(0xFF333333)) : BorderSide.none),
      child: ListTile(
  ...
  Widget _buildDefaultAvatar(String name, bool isDark) {
    return Container(
      ...
      child: Center(
        child: Text(name.isNotEmpty ? name[0].toUpperCase() : '?', style: GoogleFonts.poppins(color: skyBlue, fontSize: 32.sp, fontWeight: FontWeight.bold)),
      ),
    );
  }
  ...
  void _copyAffiliateId(BuildContext context, String id) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('ID কপি করা হয়েছে!'), behavior: SnackBarBehavior.floating));
  }
