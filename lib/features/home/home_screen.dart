...
class BannerItem {
  ...
  /// API JSON থেকে BannerItem তৈরি করে
  factory BannerItem.fromJson(Map<String, dynamic> json) {
    return BannerItem(
      id: json['id']?.toString() ?? '',
      // API তে image_url বা image যেকোনো field name হতে পারে
      imageUrl: json['image_url'] ?? json['image'] ?? json['banner_image'] ?? '',
      title: json['title'],
      subtitle: json['subtitle'],
      // API তে #RRGGBB ফরম্যাটে color আসলে Flutter Color-এ কনভার্ট
      bgColor: json['bg_color'] != null
          ? Color(int.tryParse(json['bg_color'].toString().replaceAll('#', '0xFF')) ?? 0xFF6366F1)
          : null,
      route: json['route'] ?? json['link'],
    );
  }
}
...
// ==================== BANNER API PROVIDER ====================
/// এডমিন প্যানেল থেকে ব্যানার লোড করে
...
///   ]
/// }
final bannerProvider = FutureProvider<List<BannerItem>>((ref) async {
  const String baseUrl = "https://api.easysarvice.com/api";
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (data['status'] == 'success' && data['banners'] is List) {
        final List<dynamic> bannersJson = data['banners'];
        return bannersJson.map((json) => BannerItem.fromJson(json)).toList();
      }
    }

    debugPrint("Banner API Error: ${response.statusCode} - ${response.body}");
    // API ফেল হলে খালি লিস্ট রিটার্ন (fallback)
    return [];
  } catch (e) {
    debugPrint("Banner Fetch Error: $e");
    // ইন্টারনেট বা অন্য কোনো এরর হলে খালি লিস্ট
    return [];
  }
});

// ==================== ADMIN BANNER PROVIDER (alternative) ====================
/// যদি এডমিন প্যানেলের আলাদা endpoint থাকে, যেমন: /api/admin/banners
/// এই provider টি ব্যবহার করতে পারেন
final adminBannerProvider = FutureProvider<List<BannerItem>>((ref) async {
  const String baseUrl = "https://api.easysarvice.com/api";
  final prefs = await SharedPreferences.getInstance();
  final token = prefs.getString('jwt_token');
  ...
  try {
    ...
    debugPrint("Admin Banner API Error: ${response.statusCode} - ${response.body}");
    return [];
  } catch (e) {
    debugPrint("Admin Banner Fetch Error: $e");
    return [];
  }
});
...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  void _startAutoSlide() {
    Future.delayed(const Duration(seconds: 3), () {
      ...
      // FutureProvider থেকে banners লোড হচ্ছে কিনা চেক
      final bannersAsync = ref.read(bannerProvider);
              ...
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      // ==================== BANNER SLIDER (API থেকে) ====================
                      bannersAsync.when(
                        data: (banners) {
                          if (banners.isEmpty) {
                            // API থেকে কিছু পাওয়া যায়নি — fallback placeholder দেখান
                            return Container(
                              height: isDesktop ? 180.h : isTablet ? 140.h : isSmall ? 100.h : 120.h,
                    ...
                    children: [
                      Image.network(
                        ...
                        loadingBuilder: (context, child, loadingProgress) => loadingProgress == null
                            ? child
                            : Container(
                                color: banner.bgColor ?? Theme.of(context).primaryColor,
                                child: Center(child: CupertinoActivityIndicator(radius: 12.r)),
                              ),
                        errorBuilder: (context, error, stackTrace) => Container(
                          color: banner.bgColor ?? Colors.grey,
